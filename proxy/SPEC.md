...
- **Timeout.** The tier-3 call scopes its own 15 s per-request context deadline (more headroom than the shared 8 s rerank client timeout; still well under the 30 s `identifyUpstreamTimeout`), without mutating the shared `VisionClient` client timeout the rerank / disambiguation paths depend on.

...
|---|---|---|
| ≥1 candidate resolves to catalog | pick the in-catalog candidate with the **highest `confidence`** (**rule B**), reorder it to `suggestions[0]`, set its `plant_id` | `<base>-catalog` |
| 0 in catalog, `vision != nil`, AI species resolves to catalog AND `aiConf ≥ 0.55` | replace the result with the single AI suggestion (its own confidence, `plant_id` set) | `ai-catalog-recovery` |
| 0 in catalog, AI no catalog hit / `aiConf < 0.55` / vision err, **`len(cands) > 0`** | keep the engine's **original top** candidate as `suggestions[0]` (out-of-catalog, `plant_id: null` → iOS enrichment). The AI guess is **discarded** — the engine is the trusted identifier; AI was only a catalog-recovery probe | `<base>-raw-oob` |
...
This subsumes the prior tier-3 "zero suggestions → AI" block (the zero-engine case is the last two rows). It is **independent of `ai_enhance`** (not a rerank) and requires no client opt-in.

- **Rule B accepted tradeoff.** "Highest-confidence *in-catalog*" can promote a **low-score in-catalog** candidate over a **higher-score out-of-catalog** one. This is **intentional** (user-chosen) — curated plants have reviewed toxicity / 4-scene images / care, far better UX than LLM enrichment, so maximizing curated hits outweighs strictly honoring the engine's raw ranking. See §7.
- **`0.55` threshold (`aiCatalogRecoveryMinConfidence`).** The AI catalog-recovery guess is adopted only at self-reported confidence ≥ 0.55 — a deliberate "more-likely-than-not + margin" bar so a weak AI hallucination cannot override the engine's own (out-of-catalog) answer. Below it, the engine top is kept (or, if the engine returned zero, the AI guess is still used so a result exists).

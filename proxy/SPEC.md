...
- Accept image upload from the iOS client and identify the plant via a **catalog-preference cascade** whose two goals are (i) `/v1/identify` **ALWAYS returns a result** and (ii) it **MAXIMIZES curated-catalog (1522) hits** — curated plants have reviewed toxicity / 4-scene images / care data, so an in-catalog answer beats an out-of-catalog one (which falls to lower-quality LLM enrichment). **Engine tier:** **(1) Pl@ntNet (primary) → (2) Plant.id (fallback, only when Pl@ntNet is *unavailable*)**. Pl@ntNet is tried first; only when Pl@ntNet is *unavailable* (5xx / timeout / network / 429 quota / auth-misconfig / malformed body) does the handler retry the same image against Plant.id. Pl@ntNet returning a valid answer — *including "no match found"* — is authoritative and does **not** trigger the Plant.id fallback (it is a real result, and Plant.id credit must not be spent on it). **Selection tier (NEW — runs only when an engine responded, `err == nil`):** the engine is asked for **up to 10 candidates** (`nb-results=10`) and the handler evaluates the FULL set, not just the top one — if **any** candidate resolves to the 1522 catalog it picks the in-catalog candidate with the **highest engine confidence** (rule B) and makes it `suggestions[0]`; if **none** is in catalog **and the engine's own top candidate is *not* highly confident (`confidence < 0.80`)** it asks **GPT-4o vision** to recover a catalog match (adopted only if the AI species resolves to the catalog AND its self-reported confidence ≥ `0.10` — a deliberately very low bar, user-chosen to maximize curated-library hits); **if none is in catalog but the engine's top is confident (`≥ 0.80`) the AI probe is SKIPPED** and the engine top is used as the out-of-catalog answer (a curated match is unlikely; the probe is not worth the latency/cost). Otherwise (AI no catalog hit / low conf / vision err) it keeps the engine's own top candidate as the out-of-catalog answer (the engine is the trusted identifier — the AI guess is **not** used as a re-identification), and if the engine returned zero candidates the AI guess (any confidence) is used so a result is always present. A both-engines-*unavailable* case keeps `err != nil` and still returns `502` — the AI **never** substitutes for engine-unavailable. The AI suggestion (when used) carries its **own model-reported confidence** and is **NOT flagged / marked in any special way** (product decision: AI provenance is not surfaced — same stance as the `/v1/diagnose` AI disease fallback). The AI catalog-recovery step is **free for everyone** (no paywall, no new endpoint) and **graceful**: any vision error, or no OpenAI key configured, degrades to the engine top (or, when the engine also returned nothing, the prior "We couldn't identify this plant" empty result). All paths normalize to the identical `IdentifyResult` shape; each suggestion's `scientific_name` is resolved to a YardMate `plantId` (same catalog resolver as `/v1/diagnose`, §2.1) and the **response is trimmed to ≤3 suggestions** with the chosen candidate at `[0]` (selection happened across all 10). The iOS contract is **unchanged** (same `IdentifyResult` JSON; navigates `[0]`; no client change). Cascade order + rationale in §7.
...
- Apply LLM processing when the operator-configured OpenAI key is present: (a) *optional* rerank of Plant.id top-N candidates on `/v1/identify?ai_enhance=true`; (b) *automatic* tier-3 single-shot vision identify on `/v1/identify` when the Pl@ntNet→Plant.id cascade returned zero suggestions (§2.1, free, always-on, image-conditioned `json_schema` strict output); (c) disambiguate Plant.id disease names that don't match the YardMate catalog on `/v1/diagnose`. All three keep the OpenAI key server-side only and are single-shot structured calls (not chat / conversation).
...
| `VisionClient.IdentifyPlant(...)` | `*Suggestion` (Name/ScientificName = binomial, CommonNames non-nil, Confidence = model's own 0..1; PlantID/ImageURL nil — handler fills PlantID) or `ErrVisionIdentifyUnavailable` (handler keeps the empty result on error → unchanged "can't identify") | network / non-200 / decode / model refusal / empty scientific_name (all wrapped in the one sentinel) |
| HTTP `/v1/identify` | 200 JSON (see §2.1) | 4xx/5xx per §3 |
| HTTP `/v1/diagnose` | 200 JSON (see §2.2) | 4xx/5xx per §3 |

### 1.5 External dependencies

- **Pl@ntNet API v2 (primary identify engine)** — `POST https://my-api.plantnet.org/v2/identify/all?api-key=<KEY>&lang=en&nb-results=10` (multipart with parallel `images` file part + `organs` text part; key is a query param, not a header). `nb-results=10` (not 5) so the handler can scan the full candidate set for a curated-catalog match (§2.1 catalog-preference cascade). Free tier ≈ 500 req/day; quota exhaustion returns 429 → triggers the Plant.id fallback. A no-match returns HTTP 404 `{"error":"Not Found","message":"Species not found"}` — treated as a *valid empty result*, not an engine failure.
...
- **OpenAI chat-completions (vision)** — `POST https://api.openai.com/v1/chat/completions` with `gpt-4o-2024-08-06`. Used for `ai_enhance` rerank (multimodal), the tier-3 identify fallback (multimodal, `json_schema` strict structured output), and `/v1/diagnose` catalog-id disambiguation (text-only). Shared 8 s client timeout for rerank/disambiguation; the tier-3 identify call scopes its OWN 15 s per-request context deadline (a single-shot vision identify with no candidate list to anchor on needs more headroom; still well under the handler's 30 s `identifyUpstreamTimeout`) AND runs on a dedicated ~18 s `http.Client` (`visionIdentifyClientTimeout`) so that 15 s context is the real deadline — `http.Client.Timeout` is a hard cap Go enforces independently of the context, so sending it through the shared 8 s client would clamp it to 8 s. The shared 8 s client (rerank/disambiguation) is unchanged.
...
Server returns **top 3 suggestions max** to keep payload bounded. Selection happens across the FULL engine candidate set (up to 10 — see the catalog-preference cascade below); the response is then trimmed to 3 with the **chosen candidate at `suggestions[0]`** plus the next 2 by `confidence`. iOS navigates `suggestions[0]`; the shape is unchanged.
...
**Catalog-preference selection cascade** (always-on, free, no new endpoint):

The engine is asked for **up to 10 candidates** (`nb-results=10`; `toIdentifyResult` on **both** the Pl@ntNet and Plant.id paths now caps at 10 instead of 3, so the handler sees the full set; the response is trimmed to 3 *after* selection). When the Pl@ntNet→Plant.id cascade **succeeds** (`err == nil` — engine responded; a valid no-match with zero suggestions is a success), the handler resolves `ContentIndex.LookupPlantID(c.scientific_name)` for **every** candidate and applies this decision table (`cands` = the engine candidate set, possibly empty; `<base>` = `plantnet` or `plantid-fallback`, whichever engine produced `cands`):

The table is evaluated **top-to-bottom; the first matching row wins** (the order matters — rule B precedes the confident-skip, which precedes the AI probe):

| Case | Action | `engine=` tag |
|---|---|---|
| ≥1 candidate resolves to catalog | pick the in-catalog candidate with the **highest `confidence`** (**rule B**), reorder it to `suggestions[0]`, set its `plant_id`. **Takes precedence even over a high-confidence out-of-catalog top — no threshold here (unchanged from #20)** | `<base>-catalog` |
| 0 in catalog AND the engine's **ORIGINAL top** candidate (`cands[0]`, *before* any reorder) `confidence ≥ 0.80` | use the engine's original top as-is (out-of-catalog, `plant_id: null` → iOS enrichment). **The AI catalog-recovery probe is SKIPPED entirely — `vision` is NOT called** (the engine is sure of a specific out-of-catalog plant; a curated match is unlikely and the probe is not worth the GPT-4o latency/cost) | `<base>-confident-oob` |
| 0 in catalog, engine top `< 0.80` (incl. no top), `vision != nil`, AI species resolves to catalog AND `aiConf ≥ 0.10` | replace the result with the single AI suggestion (its own confidence, `plant_id` set) | `ai-catalog-recovery` |
| 0 in catalog, engine top `< 0.80`, AI no catalog hit / `aiConf < 0.10` / vision err, **`len(cands) > 0`** | keep the engine's **original top** candidate as `suggestions[0]` (out-of-catalog, `plant_id: null` → iOS enrichment). The AI guess is **discarded** — the engine is the trusted identifier; AI was only a catalog-recovery probe | `<base>-raw-oob` |
| 0 in catalog, AI no catalog hit / low conf, **`len(cands) == 0`** but an AI guess exists (any confidence) | use the AI guess as the single suggestion (`plant_id: null`) — preserves the "always a result" guarantee for the zero-suggestion case | `ai-raw-oob` |
| `len(cands) == 0` and vision nil / err | unchanged: empty `suggestions` → iOS "We couldn't identify this plant" | `<base>` |

This subsumes the prior tier-3 "zero suggestions → AI" block (the zero-engine case is the last two rows; the `<base>-confident-oob` skip **never** applies to an empty set — there is no `cands[0]` so the gate is treated as not met → the AI path runs). It is **independent of `ai_enhance`** (not a rerank) and requires no client opt-in.

- **Rule B accepted tradeoff.** "Highest-confidence *in-catalog*" can promote a **low-score in-catalog** candidate over a **higher-score out-of-catalog** one — and this still **takes precedence even over a very-high-confidence out-of-catalog top** (the `0.80` confident-skip below applies *only* when nothing is in catalog). This is **intentional** (user-chosen) — curated plants have reviewed toxicity / 4-scene images / care, far better UX than LLM enrichment, so maximizing curated hits outweighs strictly honoring the engine's raw ranking. See §7.
- **`0.80` confident-skip gate (`plantnetConfidentSkipAIConfidence`).** When **no** candidate is in catalog but the engine's **own original top** self-reports confidence ≥ 0.80, the engine is treated as sure of a specific out-of-catalog plant: the AI catalog-recovery probe is **skipped entirely** (no GPT-4o call, lower latency/cost) and the engine top is used as the out-of-catalog answer. Edge: `len(cands) == 0` ⇒ no top ⇒ gate not met ⇒ the AI path still runs (the skip never starves the zero-engine "always a result" guarantee).
- **`0.10` threshold (`aiCatalogRecoveryMinConfidence`).** When the engine was **not** confident (top `< 0.80`) and the AI catalog-recovery guess resolves to a 1522-catalog plant, it is adopted at self-reported confidence ≥ **0.10** — a deliberately **very low** bar. The user wants to MAXIMIZE curated-library hits and explicitly accepts the tradeoff: a curated match (even at low AI confidence) is preferred over a likely-worse out-of-catalog enrichment, at the cost of occasional wrong-but-curated answers (the "rich library" feel is worth it — see §7, do **not** re-debate). Below 0.10 the engine top is kept (or, if the engine returned zero, the AI guess is still used so a result exists).
- **Single-shot, image-conditioned, structured.** The AI recovery call is one chat-completions call; the image is the only user content (treated strictly as data, anti-injection); `response_format` is a `json_schema` **strict** object `{ scientific_name: string, common_names: string[], confidence: number }` (all `required`, `additionalProperties: false`). Not a chat / conversation. The OpenAI key stays server-side only.
- **The AI suggestion is NOT flagged.** When used it is a normal `Suggestion` with its **own model-reported `confidence`** (clamped to 0..1); `is_plant` is true and `is_plant_confidence` mirrors it. There is **no provenance field** distinguishing it from a Pl@ntNet/Plant.id suggestion (product decision — consistent with the `/v1/diagnose` AI disease fallback). `ai_enhanced_at` stays `null` (this is not the `ai_enhance` rerank).
- **plant_id always resolved.** The per-suggestion `scientific_name → plantId` resolver (below) re-runs over whatever ends up in `suggestions` (idempotent), so the chosen `[0]` always carries its resolved `plant_id`; `image_url` is `null` on the AI path (no reference image).
- **Graceful degrade.** Any vision failure (network / non-200 / decode / model refusal / empty `scientific_name`, all `ErrVisionIdentifyUnavailable`), or **no OpenAI key** (`vision == nil`), degrades to the engine's top candidate (out-of-catalog) — or, when the engine also returned nothing, the unchanged `suggestions: []` "can't identify" UX. The cascade **never** fires the AI on an upstream *error*: a both-engines-*unavailable* case keeps `err != nil` and still returns `502 plant_id_unavailable` (the AI must not mask an outage). No crash, no new error code, never surfaced to the client (consistent with §3).
- **Timeout.** The AI recovery call scopes its own 15 s per-request context deadline (more headroom than the shared 8 s rerank client timeout; still well under the 30 s `identifyUpstreamTimeout`) and runs on a **dedicated ~18 s `http.Client`** so that 15 s context is the effective deadline — `http.Client.Timeout` is a hard cap Go applies independently of the context, so reusing the shared 8 s client would silently clamp the call to 8 s. The shared `VisionClient` 8 s client the rerank / disambiguation paths depend on is left unchanged.
- **Response trim.** After selection (across up to 10) + the per-suggestion `plant_id` resolver, `suggestions` is trimmed to **top-3**: `[0]` = the chosen candidate (never dropped), then the next 2 of the remainder by `confidence`. The `ai-catalog-recovery` / `ai-raw-oob` paths already hold exactly 1 suggestion (no-op trim). iOS contract unchanged.
...
Note: the wire code stays `plant_id_unavailable` / `plant_id_unauthorized` (not renamed) so the iOS error mapping is unchanged — it now denotes "all identification engines down", not literally Plant.id. OpenAI vision failures are **never** surfaced to the client. `ai_enhance` rerank failures leave `ai_enhanced_at: null` in a 200 response; the **AI catalog-recovery step** of the catalog-preference cascade failing (or no OpenAI key) degrades to the engine's own top candidate (or, when the engine also returned zero, the unchanged 200 `suggestions: []` "can't identify" UX) — it never changes a status code, and it does **not** fire on an upstream error (a both-engines-*unavailable* case still returns `502 plant_id_unavailable`; the AI never substitutes for engine-unavailable); `/v1/diagnose` catalog disambiguation failures leave `catalogId: null` on the issue. All are warn-logged server-side. The success log line carries `engine=<tag>` (`plantnet` / `plantnet-catalog` / `plantnet-confident-oob` / `plantnet-raw-oob` / `plantid-fallback` / `plantid-fallback-catalog` / `plantid-fallback-confident-oob` / `plantid-fallback-raw-oob` / `ai-catalog-recovery` / `ai-raw-oob`) + `catalogHit=<bool>` for observability.
...
| Hard 30 s timeout on the upstream Plant.id call; 8 s on OpenAI vision rerank/disambiguation; ~18 s client + 15 s context on the tier-3 vision identify | slow-loris / upstream hang exhausting our goroutines |
...
- **Tier-3 OpenAI GPT-4o vision identify fallback on `/v1/identify` (V1.0).** Decided 2026-05-18. Pl@ntNet→Plant.id can *succeed* yet return zero suggestions (Pl@ntNet valid no-match; Plant.id unfunded so it never matches), and identify is a core feature — shipping "We couldn't identify this plant" on every such request is a product gap. Tier-3 asks GPT-4o vision for a single best-guess species so the cascade goal becomes **always return a result**. Locked sub-decisions (don't re-debate): **(a)** extend the EXISTING `proxy/openai_vision.go` `VisionClient` (server-side vision proxy; reuse its transport / auth / `dataURL` image-part shape) — **no new endpoint**, no resurrected `/v1/ai-chat`, key stays server-side; **(b)** **free for everyone, no paywall** (it is a reliability floor, not a premium feature); **(c)** the AI suggestion carries its **own model-reported confidence** and is **NOT flagged / no provenance surfaced** — returned as a normal `Suggestion`, byte-identical wire shape to a Pl@ntNet/Plant.id one (same stance as the diagnose AI disease fallback; iOS needs **no change**); **(d)** **single-shot `json_schema` strict** structured output, image-conditioned, anti-injection — not chat; **(e)** never masks an upstream error (both-engines-down stays `502 plant_id_unavailable`); **(f)** fully **graceful** — any vision error or `vision == nil` (no OpenAI key) leaves the prior empty-result behavior exactly unchanged; **(g)** its own **15 s per-request context deadline** (vs the shared 8 s rerank client timeout — a candidate-less single-shot identify needs more headroom; under the 30 s handler cap) without mutating the shared client timeout. Because `http.Client.Timeout` is a hard cap Go enforces independently of the context, IdentifyPlant runs on a **dedicated ~18 s `http.Client`** (`visionIdentifyClientTimeout`, just above the 15 s context so the context is the real deadline) — not the shared 8 s client, which would otherwise clamp it to 8 s; the shared 8 s rerank/disambiguation client stays unchanged. `/v1/diagnose` is unaffected. Operator keeps `OPENAI_API_KEY` funded; a missing key degrades gracefully (warn-logged) — it does not hard-fail identify. **Superseded/subsumed 2026-05-19 by the catalog-preference cascade (next entry), then tuned the same day:** the AI-vision call is now the *catalog-recovery step* (fires when no engine candidate is in catalog AND the engine's own top is *not* confident, not only when the engine returned zero) — all of (a)–(g) still hold; the only change is *when* it fires (skipped when the engine top `≥ 0.80`) and the adoption gate, which after the 2026-05-19 cascade-tune is `aiConf ≥ 0.10` (originally `≥ 0.55`; lowered to maximize curated-library hits).**
- **Catalog-preference selection cascade on `/v1/identify` (V1.0).** Decided 2026-05-19. The server used to take the engine's `suggestions[0]` (top) and iOS navigated to it — so if a *lower-ranked* engine candidate WAS in the curated 1522 catalog (reviewed toxicity / 4-scene images / care data) we missed it and the user got worse out-of-catalog LLM-enrichment data. New behavior maximizes curated hits. **Locked sub-decisions (don't re-debate):** **(1)** request **`nb-results=10`** and raise BOTH `toIdentifyResult` caps (Pl@ntNet *and* Plant.id) 3→10 so the handler evaluates the FULL set; **(2)** **rule B** — when ≥1 candidate is in catalog, pick the in-catalog candidate with the **highest engine `confidence`** (not the engine's overall top, not the lowest-rank-in-catalog). **Explicitly accepted tradeoff (user chose B knowingly):** a **low-score in-catalog** candidate **can override a higher-score out-of-catalog** one — curated data quality outweighs strictly honoring the engine's raw ranking; do **not** re-litigate this as a "bug"; **(3)** when **no** candidate is in catalog, the AI-vision step (above) is the **catalog-recovery probe**; its guess is adopted as the in-catalog answer **only if** it resolves to a catalog `plantId` **AND** its self-reported confidence **≥ `aiCatalogRecoveryMinConfidence`** (named const) — **value tuned 2026-05-19, see the cascade-tune note below**; **(4)** if AI gives no catalog hit / low conf / errors **and the engine returned candidates**, keep the engine's **original top** (out-of-catalog, `plant_id: null` → iOS enrichment) — the **engine is the trusted identifier**, the AI guess is **discarded** here (it is a catalog probe, not a re-identification); **(5)** if the engine returned **zero** candidates, the AI guess (any confidence) is still used so the "always a result" guarantee (#18 / tier-3) holds; **(6)** a both-engines-*unavailable* case keeps `err != nil` → still `502 plant_id_unavailable` — **the AI never substitutes for engine-unavailable** (unchanged from tier-3 sub-decision (e)); **(7)** response is trimmed to **≤3** *after* selecting across 10, with the chosen candidate at `[0]` + next 2 by `confidence` — **iOS contract byte-unchanged** (same shape, navigates `[0]`); **(8)** the success log adds `engine=<tag>` (`*-catalog` / `*-confident-oob` / `*-raw-oob` / `ai-catalog-recovery` / `ai-raw-oob` / `<base>`) + `catalogHit=<bool>` for observability. `/v1/diagnose` is unaffected (it identifies exactly one plant, keeps its single top-level `plantId`).
- **Cascade tune (2026-05-19, follow-up to the entry above — locked, don't re-debate).** The user made an explicit policy call: **maximize the curated-1522 "rich library" feel even at some accuracy cost.** Two changes, nothing else: **(T1) skip the AI catalog-recovery probe when the engine is already confident about an out-of-catalog plant.** When **0** candidates are in catalog AND the engine's **ORIGINAL top** candidate (`cands[0]`, *before* any reorder) self-reports `confidence ≥ plantnetConfidentSkipAIConfidence = 0.80` (new named const), the engine top is used as-is (out-of-catalog → iOS enrichment) and **`vision` is NOT called** (new `engine=<base>-confident-oob` tag). Rationale: when the engine is this sure of a specific out-of-catalog species a curated match is unlikely, so the GPT-4o probe is not worth the latency/cost. Edge: `len(cands) == 0` ⇒ no top ⇒ gate not met ⇒ the AI path **still runs** (the skip never starves the zero-engine "always a result" guarantee). **Rule B (sub-decision (2)) is UNCHANGED and still takes precedence — a ≥1-in-catalog result wins even over a high-confidence out-of-catalog top; the `0.80` skip applies *only* when nothing is in catalog.** **(T2) lower `aiCatalogRecoveryMinConfidence` `0.55 → 0.10`.** Once the engine was **not** confident (top `< 0.80`) and the free AI identification resolves to a 1522-catalog plant, accept it on a **very low** confidence bar — curated breadth/quality is preferred over a likely-worse out-of-catalog enrichment. **Explicit accepted tradeoff (user chose this knowingly): the app will sometimes show a wrong-but-curated plant; the "rich library" feel is judged worth it. Do NOT re-litigate this as an accuracy "bug."** Everything else (rule-B precedence, the `502`/both-engines-down path, wire error codes, the AI-not-flagged / single-shot-json_schema / graceful-degrade / dedicated-client-timeout sub-decisions, the iOS `IdentifyResult` shape — ≤3, navigate `[0]`) is **UNCHANGED**.

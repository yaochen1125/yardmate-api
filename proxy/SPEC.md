...
- Accept image upload from the iOS client and identify the plant via a **three-tier cascade** whose goal is that `/v1/identify` **ALWAYS returns a result**: **(1) Pl@ntNet (primary) → (2) Plant.id (fallback, only when Pl@ntNet is *unavailable*) → (3) OpenAI GPT-4o vision (last-resort fallback, only when tiers 1–2 succeeded but yielded ZERO suggestions)**. Pl@ntNet is tried first; only when Pl@ntNet is *unavailable* (5xx / timeout / network / 429 quota / auth-misconfig / malformed body) does the handler retry the same image against Plant.id. Pl@ntNet returning a valid answer — *including "no match found"* — is authoritative and does **not** trigger the Plant.id fallback (it is a real result, and Plant.id credit must not be spent on it). When the Pl@ntNet→Plant.id cascade *succeeds* but produces zero suggestions (a valid no-match, or Plant.id unfunded so it never matched), tier-3 asks GPT-4o vision for a single best-guess species so the client always has something to show. The tier-3 AI suggestion carries its **own model-reported confidence** and is **NOT flagged / marked in any special way** (product decision: AI provenance is not surfaced — same stance as the `/v1/diagnose` AI disease fallback). Tier-3 is **free for everyone** (no paywall, no new endpoint) and **graceful**: any vision error, or no OpenAI key configured, leaves the empty result unchanged (the prior "We couldn't identify this plant" behavior). All tiers normalize to the identical `IdentifyResult` shape; each suggestion's `scientific_name` is resolved to a YardMate `plantId` (same catalog resolver as `/v1/diagnose`, §2.1) regardless of which tier answered, then returned (`POST /v1/identify`). The iOS contract is **unchanged** (same `IdentifyResult` JSON; no client change). Cascade order + rationale in §7.
...
**Tier-3 AI-vision fallback** (always-on, free, no new endpoint):

When the Pl@ntNet→Plant.id cascade **succeeds** (no upstream error) but yields **zero suggestions** — a valid Pl@ntNet/Plant.id "no match", or Plant.id is unfunded so it never matched — the handler asks `gpt-4o-2024-08-06` (vision) to identify the plant straight from the image (`VisionClient.IdentifyPlant`), so the response always carries at least one suggestion. This is **independent of `ai_enhance`** (it is not a rerank; it fires regardless of the flag) and requires no client opt-in.

- **Single-shot, image-conditioned, structured.** One chat-completions call; the image is the only user content (treated strictly as data, anti-injection); `response_format` is a `json_schema` **strict** object `{ scientific_name: string, common_names: string[], confidence: number }` (all `required`, `additionalProperties: false`). Not a chat / conversation. The OpenAI key stays server-side only.
- **The AI suggestion is NOT flagged.** It is returned as a normal `Suggestion` with its **own model-reported `confidence`** (clamped to 0..1); `is_plant` is set true and `is_plant_confidence` mirrors that confidence. There is **no provenance field** and no marker distinguishing it from a Pl@ntNet/Plant.id suggestion (product decision — consistent with the `/v1/diagnose` AI disease fallback, which is also byte-identical to a non-AI issue). `ai_enhanced_at` stays `null` (tier-3 is not the `ai_enhance` rerank).
- **plant_id still resolved.** The per-suggestion `scientific_name → plantId` resolver (below) runs unchanged on the AI suggestion: an in-catalog AI species resolves to its YardMate `plant_id`; out-of-catalog stays `null` → the iOS enrichment path, exactly like any other suggestion. `image_url` is `null` (the AI path has no reference image).
- **Graceful degrade.** Any vision failure (network / non-200 / decode / model refusal / empty `scientific_name`, all surfaced as `ErrVisionIdentifyUnavailable`), or **no OpenAI key configured** (`vision == nil`), leaves the empty result **unchanged** — a 200 with `suggestions: []` and the iOS "We couldn't identify this plant" UX, exactly as before tier-3 existed. Tier-3 **never** fires on an upstream *error* (a both-engines-down case keeps its `502 plant_id_unavailable` — the AI must not mask an outage). No crash, no new error code, never surfaced to the client (consistent with §3 — OpenAI failures are never client-visible).
- **Timeout.** The tier-3 call scopes its own 15 s per-request context deadline (more headroom than the shared 8 s rerank client timeout; still well under the 30 s `identifyUpstreamTimeout`) and runs on a **dedicated ~18 s `http.Client`** so that 15 s context is the effective deadline — `http.Client.Timeout` is a hard cap Go applies independently of the context, so reusing the shared 8 s client would silently clamp the call to 8 s (GPT-4o vision from a raw image is often slower than that). The shared `VisionClient` 8 s client the rerank / disambiguation paths depend on is left unchanged.
...
Note: the wire code stays `plant_id_unavailable` / `plant_id_unauthorized` (not renamed) so the iOS error mapping is unchanged — it now denotes "all identification engines down", not literally Plant.id. OpenAI vision failures are **never** surfaced to the client. `ai_enhance` rerank failures leave `ai_enhanced_at: null` in a 200 response; the **tier-3 AI-vision identify fallback** failing (or no OpenAI key) leaves the cascade's empty result unchanged (200 with `suggestions: []`, the unchanged "can't identify" UX) — it only ever *adds* a result, never changes a status code, and it does **not** fire on an upstream error (a both-engines-down case still returns `502 plant_id_unavailable`); `/v1/diagnose` catalog disambiguation failures leave `catalogId: null` on the issue. All are warn-logged server-side.
